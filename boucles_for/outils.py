        ...
        for element in self.infos[self.mots[2]]:
            ...
            if element[self.mots_clefs_graph['type']].lower() in ['menu','list','liste','deroulant']:
               #dic['widget']=widgets.Dropdown(options=element[self.mots_clefs_graph['contenu']],description='')
                #print(type(element[self.mots_clefs_graph['contenu']]))
                dic['widget']=widgets.Dropdown(options=convertir_donnee(element[self.mots_clefs_graph['contenu']]),description='')
...
############################################## rajout de la création des villes ####################################
def charger_villes():
    echantillon=pickle.load(open('villes_francaises', 'rb'))
    return echantillon
